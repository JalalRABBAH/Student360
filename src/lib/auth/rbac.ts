...
export async function canAccessDemoStudent(
  session: SessionPayload,
  student: { name: string; className: string },
) {
  if (hasRole(session, ROLES.SUPER_ADMIN)) return true;
  if (isLeadership(session)) return Boolean(session.schoolId);
  if (hasRole(session, ROLES.NURSE)) return Boolean(session.schoolId);
  ...
  if (hasRole(session, ROLES.STUDENT) && session.studentId) {
    ...
    return `${ownStudent?.user.firstName ?? ""} ${ownStudent?.user.lastName ?? ""}`.trim() === student.name;
  }
  ...
  if (hasRole(session, ROLES.TEACHER) && session.teacherId) {
    const assignment = await prisma.teacherClassAssignment.findFirst({
      where: {
        ...
        class: { name: student.className.replace(/^Grade\s+/i, "") },
      },
      select: { id: true },
    });
...
export async function canAccessDemoClass(
  session: SessionPayload,
  ...
  const className = schoolClass.name.replace(/^Grade\s+/i, "");

  if (hasRole(session, ROLES.TEACHER) && session.teacherId) {
    const assignment = await prisma.teacherClassAssignment.findFirst({
      where: { teacherId: session.teacherId, class: { name: className } },
      select: { id: true },
    });
    return Boolean(assignment);
  }
  ...
  if (hasRole(session, ROLES.STUDENT) && session.studentId) {
    const student = await prisma.student.findUnique({
      where: { id: session.studentId },
      select: { currentClass: { select: { name: true } } },
    });
    return student?.currentClass?.name === className;
  }
  ...
  if (hasRole(session, ROLES.PARENT) && session.guardianId) {
    const relationship = await prisma.parentStudentRelationship.findFirst({
      where: {
        guardianId: session.guardianId,
        student: { currentClass: { name: className } },
      },
      select: { id: true },
    });
    return Boolean(relationship);
